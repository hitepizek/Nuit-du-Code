...
class Cube:
    def __init__(self, x: int, y: int, cote: str):
        self.x = x
        self.y = y
        if cote == "gauche":
            self.v, self.u = 16, 0
...
class Piece:
    def __init__(self, piece: int, cote: str):
        ...
        if piece == 0:
            ...
            self.cubes = [Cube(0 + depart, -8, cote), Cube(0 + depart, 0, cote),\
                          Cube(8 + depart, 0, cote), Cube(16 + depart, 0, cote)]
        elif piece == 1:
            depart = random.randint(0, 5) * 8 + cote_x
            self.cubes = [Cube(0 + depart, 0, cote), Cube(8 + depart, -8, cote),\
                          Cube(8 + depart, 0, cote), Cube(16 + depart, 0, cote)]
            
        elif piece == 2:
            depart = random.randint(0, 4) * 8 + cote_x
            self.cubes = [Cube(0 + depart, 0, cote), Cube(8 + depart, 0, cote),\
                          Cube(16 + depart, 0, cote), Cube(24 + depart, 0, cote)]
            
        elif piece == 3:
            depart = random.randint(0, 5) * 8 + cote_x
            self.cubes = [Cube(0 + depart, 0, cote), Cube(0 + depart, -8, cote),\
                          Cube(8 + depart, -8, cote), Cube(16 + depart, -8, cote)]
        elif piece == 4:
            depart = random.randint(0, 6) * 8 + cote_x
            self.cubes = [Cube(0 + depart, 0, cote), Cube(0 + depart, -8, cote),\
                          Cube(8 + depart, -8, cote), Cube(-8 + depart, -8, cote)]
            ...
            depart = random.randint(0, 6) * 8 + cote_x
            self.cubes = [Cube(0 + depart, 0, cote), Cube(0 + depart, -8, cote),\
                          Cube(8 + depart, -8, cote), Cube(8 + depart, -16, cote)]
...
class App:
    ...
    def draw(self):
        ...
        pyxel.bltm(0, 0, 0, 0, 128, 128)
        for piece in self.pieces:
            for cube in piece.cubes:
                pyxel.blt(cube.x, cube.y, 8, 8, 6)
        if self.piece_active is not None:
            for cube in self.piece_active.cubes:
                pyxel.bkt(cube.x, cube.y, 0, cube.u, cube.v, 6)
App()
